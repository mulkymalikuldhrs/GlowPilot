...
import type {
  Message,
  DiagnosisMessage as OnboardingMessage,
} from '@/lib/types';
...
export default function OnboardingPage() {
  ...
  const startConversation = async () => {
    ...
    try {
      ...
      const initialHistory: OnboardingMessage[] = [
        {
          role: 'user',
          content: `Nama saya ${user.displayName || 'pengguna baru'}.`,
        },
      ];
      ...
      toast({
        title: 'Error',
        description: 'Gagal memulai onboarding.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const userOnboardingMessage: OnboardingMessage = {
      role: 'user',
      content: input,
    };
