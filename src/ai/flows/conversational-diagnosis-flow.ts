...
import { DiagnosisConversationInputSchema, DiagnosisConversationOutputSchema, type DiagnosisConversationInput, type DiagnosisConversationOutput } from '@/ai/schemas/conversational-diagnosis-schemas';
...
const prompt = ai.definePrompt({
  ...
  prompt: (input) => {
    const historyText = input.currentHistory.map(h => `- ${h.role}: ${h.content}`).join('\n');
    const photoPart = input.photoDataUri ? `User has provided a photo: ${input.photoDataUri}` : '';

    const text = `You are GlowPilot, a friendly and empathetic AI dermatology assistant.
...
${photoPart ? `User has provided a photo: ` : ''}
...
`;
    ...
    if (input.photoDataUri) {
      parts.push({ media: { url: input.photoDataUri, contentType: input.photoDataUri.split(';')[0].split(':')[1] } });
    }
    return parts;
  }
});
