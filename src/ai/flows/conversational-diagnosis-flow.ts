...
/**
 * @fileOverview Conducts a conversational diagnosis for skin concerns.
 */

import { ai } from '@/ai/genkit';
import type {
  DiagnosisConversationInput,
  DiagnosisConversationOutput,
} from '@/ai/schemas/conversational-diagnosis-schemas';
import {
  DiagnosisConversationInputSchema,
  DiagnosisConversationOutputSchema,
} from '@/ai/schemas/conversational-diagnosis-schemas';
import { productCatalogTool } from '../tools/product-catalog-tool';

export type { DiagnosisConversationOutput };

export async function conductDiagnosis(
  input: DiagnosisConversationInput
): Promise<DiagnosisConversationOutput> {
...
const prompt = ai.definePrompt({
  ...
  prompt: (input) => {
    const historyString = input.currentHistory
...
5.  **Provide Diagnosis & Goal (CRUCIAL):** When 'isComplete' to true, you MUST perform the following actions in order:
