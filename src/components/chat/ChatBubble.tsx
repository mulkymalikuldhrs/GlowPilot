...
import { User, Loader2, AudioLines } from 'lucide-react';
...
export function ChatBubble({
  ...
  const renderAudioIndicator = () => {
    ...
    if (message.isGeneratingAudio) {
      return (
        <div className="flex items-center gap-1 text-xs text-primary/80 mt-2">
      ...
      return (
        ...
        >
          ...
          <span className="font-medium">
            {isPlaying ? 'Memutar...' : 'Putar ulang suara'}
          </span>
        </button>
      );
    }
  ...
  return (
    ...
    >
      ...
      <div
        className={`rounded-2xl p-3 max-w-[80%] w-fit text-sm shadow-md ${
          message.role === 'user'
            ? 'bg-primary text-primary-foreground'
            : 'bg-card glass-card border-0'
        }`}
      >
