...
} from '@/ai/schemas/product-comparison-schemas';
import {
...
): Promise<ProductComparisonOutput> {
...
const productComparisonPrompt = ai.definePrompt({
  ...
  prompt: (input) => [
    {
      text: `You are a skincare comparison copilot. Your task is to compare two skincare products based on their potential price range, user ratings, and key ingredients.
...
Product 1: ${input.product1}
Product 2: ${input.product2}`,
    },
  ],
});
